...
class SEAMesh(object):
    ...
    def _matOcean(self):
        ...
        if not self.flatModel:
            ...
            # ids = np.where(np.logical_and(hsmth < self.sealevel, hl > self.sealevel))[0]
            # hsmth[ids] = hl[ids]
        ...
        if not self.flatModel:
            ...
            # ids = np.where(np.logical_and(fillz < self.sealevel, hl > self.sealevel))[0]
            # fillz[ids] = hl[ids]
        # fillz[hl >= self.sealevel] = hl[hl >= self.sealevel]
